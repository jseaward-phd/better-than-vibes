...
from btv.custom_types import Data_Features, Label_Set
...
def pick_nearest2test(
    ...
    clf_knn = core.fit_dknn_toXy(X_train, y_train)
    full_train_test_info_residual = np.sum(
        core.prediction_info(y_test, clf_knn.predict_proba(X_test))
    )
...
def cal_info_about_test_set_in_finetune_set(
    ...
    """
    if clf is None:
        _clf = core.fit_dknn_toXy(X_train, y_train)
    else:
        _clf = deepcopy(clf)
    baseline_info = np.sum(
        core.prediction_info(
...
def order_samples_by_info(
    ...
    info = core.prediction_info(y, y_predicted)
    if info.ndim > 1:
        info = info.sum(
            1
...
def add_stratified_folds_test(
    ...
    for k, old_test_idx in enumerate(running_train):
        ...
        H = core.estimate_rateVSchance(X[running_test], y[running_test], clf=_clf)
...
def add_best_fold_first_test(
    ...
    for k in trange(iters):
        best_fold_idxs = core.order_folds(X, y, clf, running_train)[0]
        train_idx = np.append(train_idx, best_fold_idxs).astype(int)
        running_train.remove(list(best_fold_idxs))

        clf.fit(X[train_idx], y[train_idx])
        y_true = y[running_test]
        H = core.estimate_rateVSchance(X[running_test], y_true, clf=clf)
