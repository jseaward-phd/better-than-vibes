...
def extraction_rate(
    clf, X_train: Data_Features, y_train: Label_Set, n: int = 1, rate: bool = True
) -> Sequence[float]:
    """
...
def fit_dknn_toXy(
    ...
    self_exclude : bool, optional
        Whether to use the custom weight function 'dist_weight_ignore_self'
    ...
    if k is None:
        ...
        if self_exclude:
            k += 1
    weights = dist_weight_ignore_self if self_exclude else "distance"
